...
import { ref, onValue, push, set, get, update, remove, runTransaction } from 'firebase/database';
...
const SEED_EVENTS = [
  ...
  },
  {
    ...
    category: 'sports',
    emoji: '🏀',
    description: 'Watch the top basketball teams compete for the national university championship.',
...
export function EventProvider({ children }) {
  ...
  useEffect(() => {
    ...
    return () => { unsubEvents(); unsubBookings(); unsubComments(); };
  }, []);
  ...
  const bookEvent = async (userId, eventId) => {
    ...
    const result = await runTransaction(eventRef, (current) => {
      if (current && current.currentBookings < current.capacity) {
        current.currentBookings = (current.currentBookings || 0) + 1;
        return current;
      }
      return;
    });
    if (!result.committed) throw new Error('Sorry, the last ticket was just taken!');

    const bookingsRef = ref(database, 'bookings');
    const newRef = push(bookingsRef);
    const newBooking = { bookingId: newRef.key, userId, eventId, timestamp: new Date().toISOString(), attended: false };
  ...
  const addEvent = async (eventData) => {
    const eventsRef = ref(database, 'events');
    const newRef = push(eventsRef);
    await set(newRef, { eventId: newRef.key, ...eventData, currentBookings: 0, createdAt: new Date().toISOString() });
  };
  ...
  const addComment = async (eventId, userId, userName, text) => {
    const newRef = push(ref(database, `comments/${eventId}`));
    await set(newRef, { id: newRef.key, userId, userName, text, timestamp: new Date().toISOString() });
  };
  ...
  return (
    <EventContext.Provider value={{ events, bookings, comments, loading, bookEvent, cancelBooking, addEvent, deleteEvent, addComment, markAttended }}>
      {children}
